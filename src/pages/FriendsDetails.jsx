...
import { FiArrowLeft, FiBell, FiArchive, FiTrash2, FiPhone, FiMessageSquare, FiVideo, FiUsers, FiCheckCircle } from 'react-icons/fi';
  ...
  return (
    <div className="w-full min-h-screen bg-[#F8FAFC] py-10 px-4 sm:px-6 font-sans relative overflow-hidden">
      ...
      <div className="max-w-5xl mx-auto">
        <button onClick={() => navigate(-1)} className="group flex items-center text-gray-500 hover:text-[#244D3F] transition-colors mb-6 font-medium text-sm">
          <FiArrowLeft className="w-4 h-4 mr-2 group-hover:-translate-x-1 transition-transform" /> Back to all friends
        </button>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-4 flex flex-col gap-4">
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-8 flex flex-col items-center text-center">
              ...
              <h1 className="text-xl font-bold text-[#1F2937] mb-3">{friend.name}</h1>
              <div className="flex flex-col gap-2 mb-6 items-center">
                <span className={`px-3 py-1 text-[11px] font-bold rounded-full uppercase tracking-wide ${statusStyle.color}`}>
                  {statusStyle.label}
                </span>
              ...
              <p className="italic text-[#64748B] text-sm mb-2">"{friend.bio}"</p>
          ...
          <div className="lg:col-span-8 flex flex-col gap-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6">
              <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 flex flex-col items-center justify-center text-center">
                <h2 className="text-3xl font-bold text-[#244D3F] mb-1">{friend.days_since_contact}</h2>
                <p className="text-xs font-medium text-[#64748B]">Days Since Contact</p>
              </div>
              <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 flex flex-col items-center justify-center text-center">
                <h2 className="text-3xl font-bold text-[#244D3F] mb-1">{friend.goal}</h2>
                <p className="text-xs font-medium text-[#64748B]">Goal (Days)</p>
              </div>
              <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 flex flex-col items-center justify-center text-center">
                <h2 className="text-2xl font-bold text-[#244D3F] mb-1">{formattedDate}</h2>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 sm:p-8">
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-[#244D3F] font-semibold text-lg">Relationship Goal</h3>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 sm:p-8">
              <h3 className="text-[#244D3F] font-semibold text-lg mb-6">Quick Check-In</h3>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
                <button onClick={() => handleCheckIn('Call')} className="flex flex-col items-center justify-center p-4 bg-[#F8FAFC] border border-gray-100 rounded-xl hover:bg-[#E8F7ED] hover:text-[#244D3F] transition-all gap-3 group">
                  <FiPhone className="w-5 h-5 text-[#334155] group-hover:text-[#244D3F]" />
                  <span className="text-sm font-medium">Call</span>
                </button>
                <button onClick={() => handleCheckIn('Text')} className="flex flex-col items-center justify-center p-4 bg-[#F8FAFC] border border-gray-100 rounded-xl hover:bg-[#E8F7ED] hover:text-[#244D3F] transition-all gap-3 group">
                  <FiMessageSquare className="w-5 h-5 text-[#334155] group-hover:text-[#244D3F]" />
                  <span className="text-sm font-medium">Text</span>
                </button>
                <button onClick={() => handleCheckIn('Video')} className="flex flex-col items-center justify-center p-4 bg-[#F8FAFC] border border-gray-100 rounded-xl hover:bg-[#E8F7ED] hover:text-[#244D3F] transition-all gap-3 group">
                  <FiVideo className="w-5 h-5 text-[#334155] group-hover:text-[#244D3F]" />
                  <span className="text-sm font-medium">Video</span>
                </button>
                <button onClick={() => handleCheckIn('Meetup')} className="flex flex-col items-center justify-center p-4 bg-[#F8FAFC] border border-gray-100 rounded-xl hover:bg-[#E8F7ED] hover:text-[#244D3F] transition-all gap-3 group">
            ...
            {timeline.filter(entry => entry.friend === friend.name).length > 0 && (
              <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 sm:p-8">
                <h3 className="text-[#244D3F] font-semibold text-lg mb-6">Recent Activity</h3>
                <div className="flex flex-col gap-4">
                  {timeline
                    .filter(entry => entry.friend === friend.name)
                    .map((entry) => (
                    <div key={entry.id} className="flex items-start gap-4 p-4 rounded-xl bg-slate-50 border border-slate-100">
                      ...
                      <div>
                        <p className="font-semibold text-[#1F2937]">{entry.type} logged</p>
                        <p className="text-sm text-[#64748B] mt-1">{entry.date} at {entry.time}</p>
                      </div>
                    </div>
                  ))}
                </div>
