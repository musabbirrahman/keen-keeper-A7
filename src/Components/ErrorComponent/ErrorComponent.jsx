...
const ErrorComponent = ({ title, message, onRetry, className = "" }) => {
  ...
  let displayMessage =
    message ||
    "An unexpected application error has occurred. We're working on getting it fixed.";
  ...
  return (
    <div
      className={`min-h-[80vh] w-full flex items-center justify-center p-4 sm:p-8 bg-gradient-to-b from-red-50/50 to-white ${className}`}
    >
      <div className="w-full max-w-lg bg-white rounded-[2rem] shadow-2xl shadow-red-900/5 border border-red-50 p-8 sm:p-12 text-center relative overflow-hidden">
        <div
          className="absolute -top-24 -right-24 w-48 h-48 bg-red-100 rounded-full opacity-50 blur-3xl"
          aria-hidden="true"
        ></div>
        <div
          className="absolute -bottom-24 -left-24 w-48 h-48 bg-rose-100 rounded-full opacity-50 blur-3xl"
          aria-hidden="true"
        ></div>
        ...
        <div className="relative z-10">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            ...
            >
              {error ? (
                <>
                  <FiArrowLeft className="w-4 h-4 mr-2" aria-hidden="true" /> Go
                  Back
                </>
              ) : (
                <>
                  <FiRefreshCcw className="w-4 h-4 mr-2" aria-hidden="true" />{" "}
                  Try Again
                </>
              )}
            </button>
